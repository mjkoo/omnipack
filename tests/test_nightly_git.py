...
def test_retry_validates_evidence_with_selected_revision_runtime(
    ...
    def advance() -> None:
        ...
        verify.write_text(
            verify.read_text()
            .replace('VERIFIER_VERSION = "0.5.0"', 'VERIFIER_VERSION = "0.6.0"')
    ...
    assert reports[0]["verifier"]["version"] == "0.5.0"
    if defect is None:
        assert reports[1]["verifier"]["version"] == "0.6.0"
