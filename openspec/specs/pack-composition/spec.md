...
A selected build failing live verification SHALL prevent publication without
causing composition to select another project or family alternative. Standard
fallback SHALL occur when no eligible preferred dual candidate remains after
successful ingestion and deliberate exclusions, unless a pin requires one.
An entire source fetch failure SHALL still abort the build. Existing configured
release fallback within one selected project SHALL remain separate and unchanged.

#### Scenario: Preferred dual project fails metadata verification

- **WHEN** a selected dual build fails metadata verification and a standard candidate exists
