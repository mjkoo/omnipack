...
- [ ] 5.3 Write failing tests, one per path that double-reports an unreadable verification input, then fix all three. First, the five offline inputs (single, dual, deny, overlay, composition): an unreadable one yields exactly one `input_unreadable` error naming it and no `input_missing` error; test at least one pack file and deny or overlay. Second, the composition-configuration check, which today runs on a deny or overlay that did not decode and adds an `invalid_composition_config` error: skip it when deny or overlay did not decode, as it already is when the composition policy did not decode, so an unreadable deny and an unreadable overlay each yield exactly one error in total for that file, `input_unreadable`, with no `input_missing` and no `invalid_composition_config`. Third, the README, which takes a separate path: an unreadable README yields exactly one `input_unreadable` error naming it and no `catalog_invalid` error saying it is missing or unreadable. A genuinely missing input of any kind is still reported as missing, and a genuinely missing deny or overlay is likewise reported once, as `input_missing` with no `invalid_composition_config`. Verify each test fails for the expected reason first, then with `just test`
