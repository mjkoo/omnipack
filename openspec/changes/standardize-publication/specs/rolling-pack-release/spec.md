...
**Reason**: Its repair duties move into "Revision changes follow completed
content publication", which compares both the recorded digests and GitHub's
reported digests of the served assets with the run's verified pair. That
comparison lets the next verified run repair an interrupted synchronization,
including one whose next pair returns to the last completed pair, and restore
missing, mixed or hand-replaced assets without advancing the revision. Bounded
requests, retries, pending target state, post-upload readback and
ambiguous-write reconciliation are dropped, because a visible failure and the
next run are enough.

**Migration**: None. The project has no users yet.
