...
**Reason**: Replaced by "Offline verification checks the serialized entry
shape". Checking that every default key is present, which rendering's
hydration guarantees, and comparing category colours and pack settings with the
removed settings configuration only re-checked rendering's own output. GitLab
URL rules stay enforced at ingestion. The setting value-type, HTML step,
request header and `preferredApkIndex` checks carry over to the replacement.

**Migration**: None; every output that passed the old check passes the new one.

### Requirement: Offline verification checks local composition constraints

**Reason**: Replaced by "Offline verification checks rendered composition
consistency", which drops historical mappings, family and variant-scoped
denials, dual-coverage exemptions, the dual-screen overlay and the
projected-eligibility check (`ineligible_output`). That check read eligibility
declared by candidate rules, which no longer declare it.

**Migration**: None; the committed configuration uses none of the removed
features.
