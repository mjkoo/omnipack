...
- [x] 2.2 Add the five extras and architecture/version policies, with explicit candidate pins selecting each maintained extra in both variants; verify fixture builds contain each exactly once, correct categories and source URLs, exclude Aurora hw/preload variants, and preserve maintained APK and version policies when same-package, same-project upstream duplicates appear, including dual-preferred BBoi34 candidates with different settings. Verify unpinned families retain existing composition precedence and missing or invalid pinned candidates fail under existing pin validation.
...
- [x] 3.1 Add the static shared GitHub track-only entry after checking synthetic-id collisions; verify both exports contain one identical entry, numeric title extraction works without APKs, unrelated release titles fail selection, and observing a new source revision leaves generated bytes unchanged.
- [x] 3.2 Add the explicit maintainer bootstrap operation for an owned revision-zero prerelease; verify absent, existing-owned, conflicting, malformed and immutable release states with mocked GitHub operations, and verify routine publishing never bootstraps or bypasses missing-tracker verification.
