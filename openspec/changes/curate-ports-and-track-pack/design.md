...
Use the existing composition policy's explicit candidate pins to select each of the five maintained extras in both single-screen and dual-screen variants. Each pin names the extra's original candidate selector and records the rationale of preserving its maintained APK and version policy. This makes the required selection survive same-package, same-project upstream duplicates, including dual-preferred BBoi34 candidates. Preserve normal composition precedence: a valid pin wins before device preference, while unpinned families retain device preference before source ranking. Existing pin validation remains in force, including build failure if a pinned candidate disappears or is ineligible or excluded.
