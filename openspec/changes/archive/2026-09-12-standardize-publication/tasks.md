...
- [x] 1.4 Delete `config/catalogs/codm.source.json` and `config/package-ids.json`, and drop their bindings from test fixtures, including the baseline index:
...
- [x] 3.4 Guard the write-side runtime of `scripts/nightly_write.py`, which runs on the runner's preinstalled `python3` (3.12 on ubuntu-latest) while the project requires 3.14:
...
- [x] 4.1 Implement `python -m scripts.source_proposal stage`, which:
