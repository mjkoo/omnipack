...
**Reason**: Generation is stateless and runs daily. A proposal opens only when
the generated catalog differs from main, so the input hash gate and the forced
refresh that bypassed it no longer serve a purpose. README prose and policy
formatting edits no longer need a metadata-only PR.

**Migration**: `config/catalogs/codm.source.json` is deleted, and
`pack generate-source codm --force` and the workflow's force input are removed.
Dispatch the workflow manually to refresh outside the daily schedule.

### Requirement: Accepted resolution state supports reuse without accepting partial catalogs

**Reason**: Resolution state and policy fingerprints are removed, and every run
resolves every project afresh. "Resolution failures keep only unchanged
committed entries" replaces this requirement. It uses main's committed catalog
as the only record of accepted entries, and the entry the current policy renders
in place of the policy fingerprint.

**Migration**: `config/package-ids.json` is deleted. No maintainer action is
needed.
