...
**Reason**: Build-time package-ID resolution and its cache-write exception are retired.
**Migration**: Use "Build failure preserves published outputs without source mutation" and the separate readme-source-generation contract. Retained output safety and composition behavior are included in the replacement.


### Requirement: The build writes a report of what it did

**Reason**: Resolution-attempt reporting belongs to source generation, while build composition reporting remains.
**Migration**: Use "The build reports composition and committed source outcomes" and the separate readme-source-generation contract. Retained output safety and composition behavior are included in the replacement.


## Requirement: The build persists newly resolved package ids

**Reason**: Resolution has moved out of routine builds.
**Migration**: Run the catalog-generation command; accepted resolver state travels with the catalog-update PR.
